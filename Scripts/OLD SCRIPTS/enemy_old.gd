...
@onready var total_fuel_spent_label: Label = $"../FuelSpent/Sprite2D2/FuelSpent"
@onready var lost: Node = $Lost
...
func select_cassettes_for_sequence():
	for i in range(0, 3):
		...
		hand.animate_cassette_to_position(selected_cassette,Vector2(0,0))
		current_slot.icon.texture = load("res://Images/action_icons/"+selected_cassette.get_current_action()[selected_cassette.CASSETTE_SIDE_DATA.ACTION_ICON]+".png")
